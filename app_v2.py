...
while cap1.isOpened() and cap2.isOpened():
    frame_index += 1
    if next_button: flag, list_frame_pass = False, []
    ret1, frame1 = cap1.read()
    ret2, frame2 = cap2.read()
    if not ret1 or not ret2: break

    frame1, frame2 = cv2.flip(frame1, 1), cv2.cvtColor(frame2, cv2.COLOR_BGR2RGB)
    if frame_index % 20 == 0:
        ...
        if face_boxes is not None:
            for (x1, y1, x2, y2), name in zip(face_boxes, face_names):
                ...
                if name: list_frame_pass = [frame1, frame2]

    else:
        # frame1, frame2 = list_frame_pass
        pass
